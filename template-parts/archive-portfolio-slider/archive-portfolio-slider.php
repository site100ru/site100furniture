<?php
/**
 * Template part: Archive Portfolio Slider
 * 
 * Параметры:
 * $args['category']            - slug категории для фильтрации (например '06-shkafy'), если не указан - выводятся все работы
 * $args['section_title']       - заголовок секции, по умолчанию "Наши работы"
 * $args['section_description'] - описание секции (подзаголовок), по умолчанию "Представляем с гордостью!"
 * $args['background_color']    - цвет фона (например: 'bg-white', 'bg-light'), по умолчанию 'bg-white'
 * $args['posts_count']         - количество постов для вывода, по умолчанию -1 (все)
 * $args['card_type']           - тип карточки: 'approximation', 'zoom-card', 'hover-image', 'magnifier', по умолчанию 'approximation'
 * $args['show_button']         - показывать кнопку "Показать еще" (bool), по умолчанию true
 * $args['button_text']         - текст кнопки, по умолчанию 'Показать еще'
 * $args['show_filter']         - показывать фильтр по категориям (bool), по умолчанию true
 * $args['show_card_title']     - показывать заголовок карточки (bool), по умолчанию true
 */

// Устанавливаем значения по умолчанию
$category = isset($args['category']) ? $args['category'] : 'all';
$section_title = isset($args['section_title']) ? $args['section_title'] : 'Наши работы';
$section_description = isset($args['section_description']) ? $args['section_description'] : 'Представляем с гордостью!';
$background_color = isset($args['background_color']) ? $args['background_color'] : 'bg-white';
$posts_count = isset($args['posts_count']) ? $args['posts_count'] : -1;
$card_type = isset($args['card_type']) ? $args['card_type'] : 'approximation';
$show_button = isset($args['show_button']) ? $args['show_button'] : true;
$button_text = isset($args['button_text']) ? $args['button_text'] : 'Показать еще';
$show_card_title = isset($args['show_card_title']) ? $args['show_card_title'] : true;
$show_filter = isset($args['show_filter']) ? $args['show_filter'] : true;

// Генерируем уникальный ID
$unique_id = 'portfolio-slider-' . uniqid();

// Проверяем наличие работ
$check_query = new WP_Query(['post_type' => 'portfolio', 'posts_per_page' => 1]);
if (!$check_query->have_posts()) {
    wp_reset_postdata();
    return;
}
wp_reset_postdata();

// Получаем категории с работами
$valid_categories = [];
if ($show_filter) {
    $portfolio_categories = get_terms(['taxonomy' => 'portfolio-cat', 'hide_empty' => true]);
    if (!empty($portfolio_categories)) {
        foreach ($portfolio_categories as $cat) {
            $test_query = new WP_Query([
                'post_type' => 'portfolio',
                'posts_per_page' => 1,
                'tax_query' => [['taxonomy' => 'portfolio-cat', 'field' => 'term_id', 'terms' => $cat->term_id]]
            ]);
            if ($test_query->have_posts()) {
                $valid_categories[] = $cat;
            }
            wp_reset_postdata();
        }
    }
}

// Автоматическое определение ссылки кнопки
$button_link = '/portfolio/';
if (!empty($category) && $category !== 'all') {
    $term = get_term_by('slug', $category, 'portfolio-cat');
    if ($term) $button_link = get_term_link($term);
}

// Функция для вывода карточек портфолио
function render_portfolio_items($query, $card_type) {
    if (!$query->have_posts()) {
        echo '<div class="col-12 text-center py-5"><p>Нет работ в этой категории.</p></div>';
        return;
    }
    
    while ($query->have_posts()) : $query->the_post();
        global $post;
        
        // Подсчет изображений
        $images = [];
        $i = 1;
        while ($img_src = get_post_meta($post->ID, '_img-' . $i, true)) {
            $images[] = $img_src;
            $i++;
        }
        $images_count = count($images);
        ?>
        <div class="col-md-6">
            <div id="carouselExampleIndicators<?php echo $post->ID; ?>" class="carousel slide mb-4"
                data-bs-ride="carousel" data-bs-interval="999999999">
                
                <?php if ($images_count > 1) : ?>
                <div class="carousel-indicators" style="bottom: 5%;">
                    <?php foreach ($images as $index => $img) : ?>
                        <button type="button" data-bs-target="#carouselExampleIndicators<?php echo $post->ID; ?>"
                            data-bs-slide-to="<?php echo $index; ?>" 
                            <?php if ($index == 0) echo 'class="active"'; ?>
                            aria-label="Slide <?php echo ($index + 1); ?>"></button>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
                
                <div class="carousel-inner">
                    <?php foreach ($images as $index => $img_src) : ?>
                        <div class="carousel-item <?php if ($index == 0) echo 'active'; ?>" data-bs-interval="999999999">
                            <a onClick="galleryOn('gallery-<?php echo $post->ID; ?>','img-<?php echo $post->ID; ?>-<?php echo $index; ?>');">
                                <?php 
                                get_template_part('template-parts/cards/card', null, [
                                    'image' => $img_src,
                                    'card_type' => $card_type,
                                    'link' => ''
                                ]);
                                ?>
                            </a>
                        </div>
                    <?php endforeach; ?>
                </div>
                
                <?php if ($images_count > 1) : ?>
                <button class="carousel-control-prev" type="button"
                    data-bs-target="#carouselExampleIndicators<?php echo $post->ID; ?>" 
                    data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Предыдущий</span>
                </button>
                <button class="carousel-control-next" type="button"
                    data-bs-target="#carouselExampleIndicators<?php echo $post->ID; ?>" 
                    data-bs-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Следующий</span>
                </button>
                <?php endif; ?>
            </div>
        </div>
    <?php endwhile;
}
?>

<!-- Archive portfolio section -->
<section class="archive-portfolio-section-2 <?php echo esc_attr($background_color); ?>" style="padding-block: 60px;">
    <div class="container">
        <!-- Заголовок секции -->
        <div class="row">
            <div class="col text-md-center">
                <h2><?php echo esc_html($section_title); ?></h2>
                <?php if ($section_description) : ?>
                    <p class="section-description archive-portfolio mb-3"><?php echo esc_html($section_description); ?></p>
                <?php endif; ?>
                <svg width="62" height="14" viewBox="0 0 62 14" fill="currentcolor" xmlns="http://www.w3.org/2000/svg" class="svg-icon mb-5">
                    <rect x="48" width="14" height="14" rx="3" />
                    <rect x="24" width="14" height="14" rx="3" />
                    <rect width="14" height="14" rx="3" />
                </svg>
            </div>
        </div>

        <?php if ($show_filter && !empty($valid_categories)) : ?>
            <!-- Табы фильтрации -->
            <div class="row">
                <div class="col text-center mb-4 mb-lg-5">
                    <div class="nav-scroller mb-0" style="text-transform: uppercase; font-family: 'HelveticaNeueCyr-Light'; font-weight: bold;">
                        <ul class="nav tablist justify-content-lg-center d-flex m-auto align-items-center myTab" id="<?php echo $unique_id; ?>" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="<?php echo $unique_id; ?>-all-tab" data-bs-toggle="tab" 
                                    data-bs-target="#<?php echo $unique_id; ?>-all" type="button" role="tab">Все</button>
                            </li>
                            <?php foreach ($valid_categories as $cat): ?>
                                <li class="nav-item">
                                    <span class="nav-link px-0">
                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="currentColor" xmlns="http://www.w3.org/2000/svg" class="svg-icon">
                                            <rect width="6" height="6" rx="2" />
                                        </svg>
                                    </span>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="<?php echo $unique_id; ?>-<?php echo esc_attr($cat->slug); ?>-tab"
                                        data-bs-toggle="tab" data-bs-target="#<?php echo $unique_id; ?>-<?php echo esc_attr($cat->slug); ?>"
                                        type="button" role="tab"><?php echo esc_html($cat->name); ?></button>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                    <div class="d-lg-none text-center mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="40" fill="currentColor" viewBox="0 0 34 40" class="svg-icon" style="opacity: 1">
                            <path d="M30.024 16c-.638 0-1.262.191-1.79.55a3.183 3.183 0 0 0-4.8-1.6 3.158 3.158 0 0 0-4.61-1.705V3.2a3.2 3.2 0 1 0-6.4 0v22.357a.2.2 0 0 1-.13.195.194.194 0 0 1-.231-.047l-2.55-2.548a5.62 5.62 0 0 0-5.788-1.367A2.37 2.37 0 0 0 2.5 25.357l5.259 7.886a26 26 0 0 0 2.754 3.474A10.79 10.79 0 0 0 18.34 40h4a10.925 10.925 0 0 0 10.192-6.901c.462-1.152.699-2.382.697-3.624V19.2a3.2 3.2 0 0 0-3.205-3.2m1.6 13.475a8.1 8.1 0 0 1-.582 3.029 9.33 9.33 0 0 1-8.707 5.896h-4a9.2 9.2 0 0 1-6.667-2.783 24.5 24.5 0 0 1-2.583-3.26L3.831 24.47a.774.774 0 0 1 .394-1.161 4.04 4.04 0 0 1 4.16.979l2.55 2.549a1.81 1.81 0 0 0 3.093-1.28V3.2a1.6 1.6 0 0 1 3.2 0v14.4a.8.8 0 0 0 1.6 0V16a1.6 1.6 0 0 1 3.2 0v2.4a.8.8 0 0 0 1.6 0v-.8a1.6 1.6 0 1 1 3.2 0V20a.8.8 0 0 0 1.6 0v-.8a1.6 1.6 0 0 1 3.2 0z" />
                            <path d="M22.101 6.4h6.069l-1.835 1.833a.801.801 0 0 0 1.133 1.133l3.2-3.2a.8.8 0 0 0 0-1.133l-3.2-3.2a.8.8 0 1 0-1.133 1.133L28.17 4.8h-6.069a.8.8 0 1 0 0 1.6M4.501 9.6a.8.8 0 0 0 .567-1.367L3.233 6.4H9.3a.8.8 0 1 0 0-1.6H3.233l1.835-1.834a.801.801 0 1 0-1.133-1.133l-3.2 3.2a.8.8 0 0 0 0 1.133l3.2 3.2a.8.8 0 0 0 .566.234" />
                        </svg>
                    </div>
                </div>
            </div>

            <!-- Контент табов -->
            <div class="tab-content">
                <!-- Все работы -->
                <div class="tab-pane fade show active" id="<?php echo $unique_id; ?>-all" role="tabpanel">
                    <div class="row">
                        <?php
                        $query_all = new WP_Query(['post_type' => 'portfolio', 'posts_per_page' => $posts_count]);
                        render_portfolio_items($query_all, $card_type);
                        $has_posts_all = $query_all->have_posts();
                        wp_reset_postdata();
                        ?>
                    </div>
                    <?php if ($has_posts_all && $show_button) : ?>
                        <div class="row text-md-center">
                            <div class="col">
                                <a href="<?php echo esc_url($button_link); ?>" class="btn btn-lg btn-corporate-color-1">
                                    <?php echo esc_html($button_text); ?>
                                </a>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>

                <!-- Табы по категориям -->
                <?php foreach ($valid_categories as $cat): ?>
                    <div class="tab-pane fade" id="<?php echo $unique_id; ?>-<?php echo esc_attr($cat->slug); ?>" role="tabpanel">
                        <div class="row">
                            <?php
                            $query_cat = new WP_Query([
                                'post_type' => 'portfolio',
                                'posts_per_page' => $posts_count,
                                'tax_query' => [['taxonomy' => 'portfolio-cat', 'field' => 'term_id', 'terms' => $cat->term_id]]
                            ]);
                            render_portfolio_items($query_cat, $card_type);
                            $has_posts_cat = $query_cat->have_posts();
                            wp_reset_postdata();
                            ?>
                        </div>
                        <?php if ($has_posts_cat && $show_button): ?>
                            <div class="row text-md-center">
                                <div class="col">
                                    <a href="<?php echo get_term_link($cat); ?>" class="btn btn-lg btn-corporate-color-1">
                                        Смотреть все
                                    </a>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>

        <?php else : ?>
            <!-- Без фильтра - обычный вывод -->
            <div class="row">
                <?php
                $query_args = ['post_type' => 'portfolio', 'posts_per_page' => $posts_count];
                if (!empty($category) && $category !== 'all') {
                    $query_args['portfolio-cat'] = $category;
                }
                $query = new WP_Query($query_args);
                render_portfolio_items($query, $card_type);
                $has_posts = $query->have_posts();
                wp_reset_postdata();
                ?>
            </div>
            
            <?php if ($has_posts && $show_button) : ?>
                <div class="row text-md-center">
                    <div class="col">
                        <a href="<?php echo esc_url($button_link); ?>" class="btn btn-lg btn-corporate-color-1">
                            <?php echo esc_html($button_text); ?>
                        </a>
                    </div>
                </div>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</section>
<!-- /Archive-portfolio section -->